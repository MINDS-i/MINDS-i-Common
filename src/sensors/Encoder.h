...
private:
    ...
    static void interrupt(void){
        stepAverage();
        if(pin[1]!=-1 && minds_i_common::utils::fastDigitalRead(pin[1])) return;
        tickCount++;
    }
    static void Binterrupt(void){
        if(minds_i_common::utils::fastDigitalRead(pin[0])) return;
        tickCount--;
    }
...
#endif // MINDS_I_COMMON_SENSORS_ENCODER_H
